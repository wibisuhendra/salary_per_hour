...
with time_range  as ( --get the list of period (yyyy-mm) from timesheets
	...
	order by year, month
), employee_  as ( --get employee data cleansed (resign date and deduplication)
...
), time_salary as ( --join the time range with the employee considering join date and resign date
...
), salary_period as ( --summarizing the salary per period
